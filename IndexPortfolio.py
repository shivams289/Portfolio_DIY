...
with st.form("my_form"):
    st.subheader('Percentage Allocation')
    col1, col2, col3, col4= st.columns(4)
    col5, col6, col7= st.columns(3)
    if domeq:
        with col1:
            n50 = st.number_input("NIFTY 50", min_value=0.0, max_value=100.0)

        with col2:
            nn50 = st.number_input("NIFTY NEXT 50", min_value=0.0, max_value=100.0)

        with col3:
            smallcap = st.number_input("NIFTY SMALLCAP 250", min_value=0.0, max_value=100.0)

        with col4:
            midcap = st.number_input("NIFTY MIDCAP 150", min_value=0.0, max_value=100.0)
    ...
    if inteq:
        with col5:
            snp500 = st.number_input("S&P 500", min_value=0.0, max_value=100.0)
    else:
        snp500 = 0

    if debt:
        with col6:
            debt_alloc = st.number_input("DEBT", min_value=0.0, max_value=100.0)
    ...
    if gold:
        with col7:
            gold_alloc = st.number_input("GOLD", min_value=0.0, max_value=100.0)
